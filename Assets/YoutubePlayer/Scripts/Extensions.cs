...
namespace YoutubePlayer
{
    public static class Extensions
    {
        ...
        public static List<VideoQuality> GetVideoQualityList(this StreamManifest streamManifest)
        {
            ...
            // Get the list of distinct video qualities from the available muxed streams
            return streamManifest.GetMuxedStreams()
                                 .Select(stream => stream.VideoQuality)
                                 .Distinct()
                                 .ToList();
        }
        ...
        public static MuxedStreamInfo WithCustomVideoQualitySupported(this StreamManifest streamManifest, VideoQuality quality)
        {
            if (streamManifest == null)
                throw new ArgumentNullException(nameof(streamManifest));
            return streamManifest.GetMuxedStreams().Where(info => info.VideoQuality == quality).Select(info => info).FirstOrDefault();
        }
        /// <summary>
